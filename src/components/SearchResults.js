...
export const SearchResults = (props) => {
    const searchResultsArray = props.searchResults.map((result) => {
        return (
            <div className="col-sm-3 col-xs-12 py-3 px-0 mx-0">
                <Link to={{ pathname: `/gif/${result.id}` }} key={result.id}>
                    <Gif
    ...
    return (
        <div className="container-fluid py-5 text-center">
            {searchResultsArray.length === 0 ? (
                ...
                <h2 className="text-white">Results for "{props.search}"</h2>
            )}
